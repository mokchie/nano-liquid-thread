...
for cn,fn in enumerate(Fn):
    ...
    for it,t in enumerate(Time):
        ...
        rave = np.average(R)
        nw = 4
        Sp = fft.fft((R-np.sqrt(np.average(R**2))))[1:nw]*dx/L#R0**2
        Rmin.append(rave-2*np.abs(Sp[0]))
        Rmax.append(rave+2*np.abs(Sp[0]))
        Rave.append(rave)
        Tt.append(t)
    ...
    print(fn,',',np.max(((Rmax-Rmin)/2/R0)))
    np.save(fp,Tt)
...
#ax1.plot(Time/1e6,hmax_ave,'r-.')
#ax1.fill_between(Time/1e6,hmax_sq_ave - hmax_std,(Rmax_ave-Rmin_ave)/2/R0 + hmax_std,alpha=0.2,color='k')
with open('2H(1)_ave_md.npy','wb') as fp:
    np.save(fp,Time/1e6)
